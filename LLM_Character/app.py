...
class EmotionDecorator(MessageProcessing):
    ...
    def get_messages(self, query : str) -> MessageStruct:        
        ...
        query = AIMessage(message='Based on the Instruction and the chat history estimate the emotion of the agent: happy, angry, disgust, fear, surprised, sad or neutral. Slightly exaggerate the emotion. No other emotions are allowed. Answer only with the emotion.', role=DEVELOPER, class_type="MessageAI", sender=DEVELOPER)
